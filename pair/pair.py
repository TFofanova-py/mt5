...
class Pair:
    ...
    def save_raw_data(self, data):
        ...
        if os.path.exists(self.data_file):
            with open(self.data_file) as f:
                ...
                last_record_dt = datetime.datetime.strptime(last_record_dt_str, "%Y-%m-%d %H:%M:%S").replace(tzinfo=pytz.UTC)
    ...
    def fetch_prices(self, session, numpoints=MIN_PRICE_HIST_PERIOD):
        ...
        assert curr_prices is not None, f"can't fetch historical data from {self.data_source}"
    ...
    def add_levels(self, df: pd.DataFrame, period: int = None, highest_fib: float = None) -> pd.DataFrame:
        ...
        # left_bars, right_bars, _ = self.support_parameters
        # window_size = left_bars + right_bars + 1
        # df["support"] = df["close"].rolling(window=window_size,
        #                                     min_periods=window_size).apply(get_pivotlevel,
        #                                                                    args=(right_bars,
        #                                                                          "low")).fillna(method="ffill")
        # df["resistance"] = df["close"].rolling(window=window_size,
        #                                        min_periods=window_size).apply(get_pivotlevel,
        #                                                                       args=(right_bars,
...
#
#
# def parse_stop_coefficient(st: str) -> Union[float, Literal["hf"]]:
#     if st == "hf":
#         return st
#     else:
#         try:
#             return float(st)
#         except TypeError as e:
#             print(e, "stop_coefficient must be a float or a string with value 'hf'")
